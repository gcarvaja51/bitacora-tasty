...
# de que el modelo elija bien el metodo). Ventana: 09:00-16:05 ET, lunes-viernes
# (30 min antes de la apertura hasta 5 min despues del cierre -- ajustado
# 2026-07-28 a pedido del usuario, coincide con la ventana ya documentada en
# el skill premercado-spx que este guard nunca reflejo del todo).
...
$dentroDeVentana = ($etNow.DayOfWeek -ne 'Saturday') -and ($etNow.DayOfWeek -ne 'Sunday') -and ($etMinutes -ge (9*60)) -and ($etMinutes -le (16*60+5))
if (-not $dentroDeVentana) {
    $msg = "Fuera de horario ($($etNow.DayOfWeek) $($etNow.ToString('HH:mm')) ET, ventana valida 09:00-16:05 ET lunes-viernes) -- guard duro de PowerShell, no se invoco a Claude."
...
5. Ademas de empujarlo a TradingView, manda estos mismos datos al servidor de produccion via PowerShell (Invoke-RestMethod), para que el servidor use la MISMA fuente que ve el usuario en vez de (o ademas de) su propio calculo interno: `Invoke-RestMethod -Uri "https://web-production-23473.up.railway.app/api/spx/sigma-levels" -Method Post -ContentType "application/json" -Body (@{netGex=<numero con signo>; netDex=<numero con signo>; regime=(<numero> -gt 0 ? "POSITIVO" : "NEGATIVO"); callWall=<call_wall>; putWall=<put_wall>; gammaFlip=<gamma_flip>; mvs=<mvs>; spxPrice=<spot>} | ConvertTo-Json)`. netDex es el Net DEX extraido en el paso 3, con el mismo signo. Si este POST falla, no es fatal -- registralo pero segui (el paso 5 a TradingView ya se hizo, es lo mas importante visualmente).
